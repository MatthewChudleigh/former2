...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("LexModelBuildingService", "getBots", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.bots.map(async (bot) => {
            return context.sdkcall("LexModelBuildingService", "getBot", {
                name: bot.name,
    ...
    await context.sdkcall("LexModelBuildingService", "getIntents", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.intents.map(async (intent) => {
            return context.sdkcall("LexModelBuildingService", "getIntent", {
                name: intent.name,
    ...
    await context.sdkcall("LexModelBuildingService", "getSlotTypes", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.slotTypes.map(async (slotType) => {
            return context.sdkcall("LexModelBuildingService", "getSlotType", {
                name: slotType.name,
    ...
    await context.sdkcall("LexModelsV2", "listBots", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.botSummaries.map(async (bot) => {
            await context.sdkcall("LexModelsV2", "describeBot", {
                botId: bot.botId
            }, true).then((data) => {
                resources.push({
                    f2id: data.botId,
            ...
            await context.sdkcall("LexModelsV2", "listBotAliases", {
                botId: bot.botId
            }, true).then(async (data) => {
                await Promise.all(data.botAliasSummaries.map(async (botalias) => {
                    return context.sdkcall("LexModelsV2", "describeBotAlias", {
                        botId: data.botId,
            ...
            await context.sdkcall("LexModelsV2", "listBotVersions", {
                botId: bot.botId
            }, true).then(async (data) => {
                await Promise.all(data.botVersionSummaries.map(async (botversion) => {
                    ...
                    return context.sdkcall("LexModelsV2", "describeBotVersion", {
                        botId: data.botId,
                        botVersion: botversion.botVersion
                    }, true).then(async (data) => {
                        await context.sdkcall("LexModelsV2", "listBotLocales", {
                            botId: data.botId,
                            botVersion: botversion.botVersion
            ...
            return context.sdkcall("STS", "getCallerIdentity", {
                // no params
