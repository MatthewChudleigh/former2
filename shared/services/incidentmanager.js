...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("SSMIncidents", "listReplicationSets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.replicationSetArns.map(arn => {
            return context.sdkcall("SSMIncidents", "getReplicationSet", {
                arn: arn
            }, true).then((data) => {
    ...
    await context.sdkcall("SSMIncidents", "listResponsePlans", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.responsePlanSummaries.map(responseplan => {
            return context.sdkcall("SSMIncidents", "getResponsePlan", {
    ...
    await context.sdkcall("SSMContacts", "listContacts", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.Contacts.map(async (contact) => {
            await context.sdkcall("SSMContacts", "getContact", {
            ...
            return context.sdkcall("SSMContacts", "listContactChannels", {
                ContactId: contact.ContactArn
            }, false).then(async (data) => {
                await Promise.all(data.ContactChannels.map(async (contactchannel) => {
                    await context.sdkcall("SSMContacts", "getContactChannel", {
