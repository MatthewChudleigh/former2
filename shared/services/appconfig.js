...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("AppConfig", "listApplications", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.Items.map(async (item) => {
            await context.sdkcall("AppConfig", "getApplication", {
            ...
            await context.sdkcall("AppConfig", "listEnvironments", {
                ApplicationId: item.Id
            }, true).then(async (data) => {
                await Promise.all(data.Items.map(async (environmentitem) => {
                    await context.sdkcall("AppConfig", "getEnvironment", {
                        ApplicationId: item.Id,
                    ...
                    return context.sdkcall("AppConfig", "listDeployments", {
                        ApplicationId: item.Id,
                        EnvironmentId: environmentitem.Id
                    }, true).then(async (data) => {
                        await Promise.all(data.Items.map(deploymentitem => {
                            return context.sdkcall("AppConfig", "getDeployment", {
                                ApplicationId: item.Id,
                                EnvironmentId: environmentitem.Id,
            ...
            return context.sdkcall("AppConfig", "listConfigurationProfiles", {
                ApplicationId: item.Id
            }, true).then(async (data) => {
                await Promise.all(data.Items.map(async (configprofileitem) => {
                    await context.sdkcall("AppConfig", "listHostedConfigurationVersions", {
                        ApplicationId: item.Id,
                        ConfigurationProfileId: configprofileitem.Id
                    }, true).then(async (data) => {
                        await Promise.all(data.Items.map(hostedconfigversion => {
                            return context.sdkcall("AppConfig", "getHostedConfigurationVersion", {
                                ApplicationId: item.Id,
                                ConfigurationProfileId: configprofileitem.Id,
                    ...
                    return context.sdkcall("AppConfig", "getConfigurationProfile", {
                        ApplicationId: item.Id,
                        ConfigurationProfileId: configprofileitem.Id
                    }, true).then(async (data) => {
                        resources.push({
    ...
    await context.sdkcall("AppConfig", "listDeploymentStrategies", {
        // no params
    }, true).then(async (data) => {
        return Promise.all(data.Items.map(deploymentstrategyitem => {
            return context.sdkcall("AppConfig", "getDeploymentStrategy", {
