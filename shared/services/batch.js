...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("Batch", "describeComputeEnvironments", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("Batch", "describeJobDefinitions", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("Batch", "describeJobQueues", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("Batch", "listSchedulingPolicies", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.schedulingPolicies.map(async (schedulingPolicy) => {
            await context.sdkcall("Batch", "describeSchedulingPolicies", {
