...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("DynamoDB", "listTables", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.TableNames.map(async (tableName) => {
            return context.sdkcall("DynamoDB", "describeTable", {
                TableName: tableName
            }, true).then(async (data) => {
                await context.sdkcall("DynamoDB", "listTagsOfResource", {
                    ...
                    await context.sdkcall("DynamoDB", "describeTimeToLive", {
                    ...
                    if (data.Table.Replicas && data.Table.Replicas.length > 0) {
                        resources.push({
                            ...
                            f2link: 'https://console.aws.amazon.com/dynamodb/home?region=' + context.region + '#tables:selected=' + data.Table.TableName,
    ...
    await context.sdkcall("DAX", "describeClusters", {
        // no params
    }, false).then((data) => {
    ...
    await context.sdkcall("DAX", "describeParameterGroups", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.ParameterGroups.map(parameterGroup => {
            return context.sdkcall("DAX", "describeParameters", {
    ...
    await context.sdkcall("DAX", "describeSubnetGroups", {
        // no params
    }, false).then((data) => {
