...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("EMR", "listClusters", {
        ClusterStates: [
            "RUNNING",
            "STARTING",
            "BOOTSTRAPPING",
        ...
        await Promise.all(data.Clusters.map(cluster => {
            return Promise.all([
                ...
                context.sdkcall("EMR", "listInstanceFleets", {
                    ClusterId: cluster.Id
                }, false).then((data) => {
                ...
                context.sdkcall("EMR", "listInstanceGroups", {
                    ClusterId: cluster.Id
                }, false).then((data) => {
                ...
                context.sdkcall("EMR", "describeCluster", {
                    ClusterId: cluster.Id
                }, true).then((data) => {
                    resources.push({
    ...
    await context.sdkcall("EMR", "listSecurityConfigurations", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.SecurityConfigurations.map(securityConfiguration => {
            return context.sdkcall("EMR", "describeSecurityConfiguration", {
    ...
    await context.sdkcall("EMRcontainers", "listVirtualClusters", {
        states: ["RUNNING"]
    }, false).then(async (data) => {
        await Promise.all(data.virtualClusters.map(async (virtualCluster) => {
            return context.sdkcall("EMRcontainers", "describeVirtualCluster", {
    ...
    await context.sdkcall("EMR", "listStudios", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.Studios.map(async (studio) => {
            await context.sdkcall("EMR", "describeStudio", {
            ...
            return context.sdkcall("EMR", "listStudioSessionMappings", {
                StudioId: studio.StudioId
            }, true).then((data) => {
    ...
    await context.sdkcall("EMRServerless", "listApplications", {
        states: ["STARTED", "CREATED"]
    }, false).then(async (data) => {
        await Promise.all(data.applications.map(async (application) => {
            return context.sdkcall("EMRServerless", "getApplication", {
