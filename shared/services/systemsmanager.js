...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("SSM", "listDocuments", {
        DocumentFilterList: [{
            key: "Owner",
            value: "Self"
        }]
        ...
        await Promise.all(data.DocumentIdentifiers.map(document => {
            return context.sdkcall("SSM", "getDocument", {
                Name: document.Name,
                DocumentFormat: 'JSON'
    ...
    await context.sdkcall("SSM", "describeParameters", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.Parameters.map(parameter => {
            return context.sdkcall("SSM", "getParameter", {
                Name: parameter.Name
            }, true).then((data) => {
    ...
    await context.sdkcall("SSM", "describePatchBaselines", {
        Filters: [{
            Key: "OWNER",
            Values: ["Self"]
        }]
        ...
        await Promise.all(data.BaselineIdentities.map(baseline => {
            return context.sdkcall("SSM", "getPatchBaseline", {
    ...
    await context.sdkcall("SSM", "listAssociations", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.Associations.map(association => {
            if (association.AssociationId) {
                return context.sdkcall("SSM", "describeAssociation", {
    ...
    await context.sdkcall("SSM", "describeMaintenanceWindows", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.WindowIdentities.map(window => {
            return Promise.all([
                context.sdkcall("SSM", "describeMaintenanceWindowTasks", {
                    WindowId: window.WindowId
                }, true).then(async (data) => {
                    await Promise.all(data.Tasks.map(task => {
                        return context.sdkcall("SSM", "getMaintenanceWindowTask", {
                            WindowId: window.WindowId,
                ...
                context.sdkcall("SSM", "describeMaintenanceWindowTargets", {
                    WindowId: window.WindowId
                }, true).then((data) => {
                ...
                context.sdkcall("SSM", "getMaintenanceWindow", {
                    WindowId: window.WindowId
                }, true).then((data) => {
                    resources.push({
                        f2id: data.WindowId,
    ...
    await context.sdkcall("SSM", "listResourceDataSync", {
        // no params
    }, true).then((data) => {
