...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("MQ", "listBrokers", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.BrokerSummaries.map(async (brokerSummary) => {
            return context.sdkcall("MQ", "describeBroker", {
                ...
                if (data.Users) {
                    ...
                    await Promise.all(data.Users.map(async (user) => {
                        return context.sdkcall("MQ", "describeUser", {
    ...
    await context.sdkcall("MQ", "listConfigurations", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.Configurations.map(configuration => {
            return context.sdkcall("MQ", "describeConfiguration", {
                ConfigurationId: configuration.Id
            }, true).then(async (data) => {
                await context.sdkcall("MQ", "describeConfigurationRevision", {
                    ConfigurationId: configuration.Id,
