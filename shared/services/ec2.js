...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("EC2", "describeInstances", {
        // no params
    }, true).then((data) => {
        data.Reservations.forEach(reservation => {
            reservation.Instances.forEach(async (instance) => {
                ...
                if (instance.BlockDeviceMappings) {
                    for (var i = 0; i < instance.BlockDeviceMappings.length; i++) {
                        if (instance.BlockDeviceMappings[i].Ebs && instance.BlockDeviceMappings[i].Ebs.VolumeId) {
                            await context.sdkcall("EC2", "describeVolumes", {
                ...
                await context.sdkcall("EC2", "describeInstanceAttribute", {
                ...
                if (instance.ElasticGpuAssociations && instance.ElasticGpuAssociations.length) {
                    ...
                    instance.ElasticGpuAssociations.forEach(async (elasticGpuAssociation) => {
                        await context.sdkcall("EC2", "describeElasticGpus", {
                ...
                resources.push({
                    ...
                    f2link: 'https://console.aws.amazon.com/ec2/home?region=' + context.region + '#Instances:search=' + instance.InstanceId,
                    name: name,
                    instanceid: instance.InstanceId,
    ...
    await context.sdkcall("EC2", "describeHosts", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("EC2", "describeCapacityReservations", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("EC2", "describeCapacityReservationFleets", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("EC2", "describeFleets", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("ELB", "describeLoadBalancers", {
        // no params
    }, true).then(async (data) => {
        data.LoadBalancerDescriptions.forEach(async (loadBalancer) => {
            loadBalancer['Tags'] = await context.getResourceTags("arn:aws:elasticloadbalancing:region:account:loadbalancer/" + loadBalancer.LoadBalancerName);

            await context.sdkcall("ELB", "describeLoadBalancerAttributes", {
                ...
                resources.push({
                    ...
                    f2data: loadBalancer,
                    f2region: context.region,
                    f2link: 'https://console.aws.amazon.com/ec2/home?region=' + context.region + '#LoadBalancers:search=' + loadBalancer.LoadBalancerName,
    ...
    await context.sdkcall("ELBv2", "describeLoadBalancers", {
        // no params
    }, true).then((data) => {
        data.LoadBalancers.forEach(async (loadBalancer) => {
            await context.sdkcall("ELBv2", "describeLoadBalancerAttributes", {
            ...
            await context.sdkcall("ELBv2", "describeListeners", {
                LoadBalancerArn: loadBalancer.LoadBalancerArn
            }, true).then(async (data) => {
                await Promise.all(data.Listeners.map(async (listener) => {
                    listener['Tags'] = await context.getResourceTags(listener.ListenerArn)
                    resources.push({
                    ...
                    return Promise.all([
                        listener.Protocol.startsWith("HTTP") ? context.sdkcall("ELBv2", "describeListenerCertificates", {
                        ...
                        context.sdkcall("ELBv2", "describeRules", {
                            ListenerArn: listener.ListenerArn
                        }, true).then((data) => {
                            data.Rules.forEach(async (rule) => {
                                rule['ListenerArn'] = listener.ListenerArn;
                                rule['Tags'] = await context.getResourceTags(rule.RuleArn);
                                resources.push({
                                    f2id: rule.RuleArn,
    ...
    await context.sdkcall("AutoScaling", "describeAutoScalingGroups", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.AutoScalingGroups.map(async (autoScalingGroup) => {
            resources.push({
                ...
                f2region: context.region,
                f2link: 'https://console.aws.amazon.com/ec2/autoscaling/home?region=' + context.region + '#AutoScalingGroups:id=' + autoScalingGroup.AutoScalingGroupName + ';filter=' + autoScalingGroup.AutoScalingGroupName,
            ...
            await context.sdkcall("AutoScaling", "describeLifecycleHooks", {
            ...
            return context.sdkcall("AutoScaling", "describeWarmPool", {
                AutoScalingGroupName: autoScalingGroup.AutoScalingGroupName
            }, true).then(async (data) => {
    ...
    await context.sdkcall("AutoScaling", "describeLaunchConfigurations", {
        // no params
    }, true).then((data) => {
        data.LaunchConfigurations.forEach(launchConfiguration => {
            resources.push({
                ...
                f2region: context.region,
                f2link: 'https://console.aws.amazon.com/ec2/autoscaling/home?region=' + context.region + '#LaunchConfigurations:id=' + launchConfiguration.LaunchConfigurationName + ';filter=' + launchConfiguration.LaunchConfigurationName,
    ...
    await context.sdkcall("EC2", "describeLaunchTemplates", {
        // no params
    }, true).then(async (data) => {
        if (data.LaunchTemplates) {
            await Promise.all(data.LaunchTemplates.map(launchTemplate => {
                return context.sdkcall("EC2", "describeLaunchTemplateVersions", {
    ...
    await context.sdkcall("ELBv2", "describeTargetGroups", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.TargetGroups.map(async (targetGroup) => {
            await context.sdkcall("ELBv2", "describeTargetHealth", {
                TargetGroupArn: targetGroup.TargetGroupArn
            }, true).then((data) => {
                targetGroup["Targets"] = data.TargetHealthDescriptions;
            }).catch(() => { });

            return context.sdkcall("ELBv2", "describeTargetGroupAttributes", {
                TargetGroupArn: targetGroup.TargetGroupArn
            }, true).then((data) => {
    ...
    await context.sdkcall("EC2", "describeVolumes", {
        ...
        data.Volumes.forEach(volume => {
            resources.push({
                ...
                f2data: volume,
                f2region: context.region,
                f2link: 'https://console.aws.amazon.com/ec2/home?region=' + context.region + '#Volumes:search=' + volume.VolumeId,
    ...
    await context.sdkcall("EC2", "describeNetworkInterfaces", {
        // no params
    }, true).then((data) => {
        data.NetworkInterfaces.forEach(networkInterface => {
            resources.push({
                ...
                f2region: context.region,
                f2link: 'https://console.aws.amazon.com/ec2/home?region=' + context.region + '#NIC:search=' + networkInterface.NetworkInterfaceId,
    ...
    context.sdkcall("EC2", "describeNetworkInterfacePermissions", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("EC2", "describeSpotFleetRequests", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("EC2", "describePlacementGroups", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("AutoScaling", "describePolicies", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("AutoScaling", "describeScheduledActions", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("EC2", "describeSecurityGroups", {
        // no params
    }, true).then((data) => {
        data.SecurityGroups.forEach(securityGroup => {
            if (
                ...
                resources.push({
                    ...
                    f2region: context.region,
                    f2link: 'https://console.aws.amazon.com/ec2/home?region=' + context.region + '#SecurityGroups:search=' + securityGroup.GroupId,
    ...
    await context.sdkcall("EC2", "describeFlowLogs", {
        // no params
    }, true).then((data) => {
        if (data.FlowLogs) {
    ...
    await context.sdkcall("DLM", "getLifecyclePolicies", {
        // no params
    }, true).then(async (data) => {
        if (data.Policies) {
            await Promise.all(data.Policies.map(policy => {
                return context.sdkcall("DLM", "getLifecyclePolicy", {
                    ...
                    resources.push({
                        ...
                        f2data: data.Policy,
                        f2region: context.region,
                        f2link: 'https://console.aws.amazon.com/ec2/v2/home?region=' + context.region + '#Lifecycle:search=' + data.Policy.PolicyId + ';sort=PolicyId',
    ...
    await context.sdkcall("ApplicationAutoScaling", "describeScalableTargets", {
        ServiceNamespace: "ec2"
    }, true).then(async (data) => {
        await Promise.all(data.ScalableTargets.map(target => {
            return context.sdkcall("ApplicationAutoScaling", "describeScheduledActions", {
                ServiceNamespace: "ec2",
    ...
    await context.sdkcall("ApplicationAutoScaling", "describeScalingPolicies", {
        ServiceNamespace: "ec2"
    }, true).then(async (data) => {
    ...
    await context.sdkcall("EC2", "describeKeyPairs", {
        // no params
    }, true).then(async (data) => {
        if (data.KeyPairs) {
    ...
    await context.sdkcall("ACM", "listCertificates", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.CertificateSummaryList.map(async (cert) => {
            return context.sdkcall("EC2", "getAssociatedEnclaveCertificateIamRoles", {
