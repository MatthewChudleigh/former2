...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("CodeArtifact", "listDomains", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.domains.map(async (domain) => {
            return context.sdkcall("CodeArtifact", "getDomainPermissionsPolicy", {
                domain: domain.name
            }, false).then(async (data) => {
                data.policy['domain'] = domain.name;

                await context.sdkcall("CodeArtifact", "describeDomain", {
                    domain: domain.name
    ...
    await context.sdkcall("CodeArtifact", "listRepositories", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.repositories.map(async (repository) => {
            return context.sdkcall("CodeArtifact", "describeRepository", {
                repository: repository.name,
                domain: repository.domainName
            }, true).then(async (data) => {
                await context.sdkcall("CodeArtifact", "getRepositoryPermissionsPolicy", {
                    repository: repository.name,
                    domain: repository.domainName
