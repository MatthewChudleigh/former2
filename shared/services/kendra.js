...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("Kendra", "listIndices", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.IndexConfigurationSummaryItems.map(async (index) => {
            return Promise.all([
                context.sdkcall("Kendra", "describeIndex", {
                    Id: index.Id
                }, true).then(async (data) => {
                    data['Tags'] = await context.getResourceTags("arn:aws:kendra:region:account:index/" + data.Id);
                ...
                context.sdkcall("Kendra", "listDataSources", {
                    IndexId: index.Id
                }, true).then(async (data) => {
                    await Promise.all(data.SummaryItems.map(async (datasource) => {
                        return context.sdkcall("Kendra", "describeDataSource", {
                            Id: datasource.Id,
                            IndexId: index.Id
                        }, true).then(async (data) => {
                            data['Tags'] = await context.getResourceTags("arn:aws:kendra:region:account:data-source/" + data.IndexId + "/faq/" + data.Id);
                ...
                context.sdkcall("Kendra", "listFaqs", {
                    IndexId: index.Id
                }, true).then(async (data) => {
                    await Promise.all(data.FaqSummaryItems.map(async (faq) => {
                        return context.sdkcall("Kendra", "describeFaq", {
                            Id: faq.Id,
                            IndexId: index.Id
                        }, true).then(async (data) => {
                            data['Tags'] = await context.getResourceTags("arn:aws:kendra:region:account:index/" + data.IndexId + "/faq/" + data.Id);
