...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("Amplify", "listApps", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.apps.map(app => {
            return Promise.all([
                context.sdkcall("Amplify", "getApp", {
                    appId: app.appId
                ...
                context.sdkcall("Amplify", "listBranches", {
                    appId: app.appId
                }, true).then(async (data) => {
                    await Promise.all(data.branches.map(branch => {
                        return context.sdkcall("Amplify", "getBranch", {
                            appId: app.appId,
                ...
                context.sdkcall("Amplify", "listDomainAssociations", {
                    appId: app.appId
                }, true).then(async (data) => {
                    await Promise.all(data.domainAssociations.map(domain => {
                        return context.sdkcall("Amplify", "getDomainAssociation", {
                            appId: app.appId,
                ...
                context.sdkcall("Amplify", "listBackendEnvironments", {
                    ...
                    await Promise.all(data.backendEnvironments.map(async (backendEnvironment) => {
                        return context.sdkcall("AmplifyUIBuilder", "listThemes", {
                            appId: app.appId,
                            environmentName: backendEnvironment.environmentName
                        }, true).then(async (data) => {
                            return Promise.all(data.entities.map(entity => {
                                return context.sdkcall("AmplifyUIBuilder", "getTheme", {
                                    appId: app.appId,
                                    environmentName: backendEnvironment.environmentName,
                                    id: entity.id
                                }, true).then(async (data) => {
