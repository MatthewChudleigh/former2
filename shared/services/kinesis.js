...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("Kinesis", "listStreams", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.StreamNames.map(streamName => {
            return context.sdkcall("Kinesis", "describeStream", {
                StreamName: streamName
            }, true).then(async (data) => {
                data.StreamDescription['Tags'] = await context.getResourceTags(data.StreamDescription.StreamARN);
                ...
                await context.sdkcall("Kinesis", "listStreamConsumers", {
                    StreamARN: data.StreamDescription.StreamARN
                }, true).then(async (data) => {
                    await Promise.all(data.Consumers.map(consumer => {
                        return context.sdkcall("Kinesis", "describeStreamConsumer", {
                            StreamARN: data.StreamDescription.StreamARN,
    ...
    await context.sdkcall("Firehose", "listDeliveryStreams", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.DeliveryStreamNames.map(deliveryStreamName => {
            return context.sdkcall("Firehose", "describeDeliveryStream", {
    ...
    await context.sdkcall("KinesisAnalytics", "listApplications", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.ApplicationSummaries.map(applicationSummary => {
            return context.sdkcall("KinesisAnalytics", "describeApplication", {
    ...
    await context.sdkcall("KinesisAnalyticsV2", "listApplications", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.ApplicationSummaries.map(applicationSummary => {
            return context.sdkcall("KinesisAnalyticsV2", "describeApplication", {
                ApplicationName: applicationSummary.ApplicationName
            }, true).then((data) => {
                resources.push({
                    f2id: data.ApplicationDetail.ApplicationARN,
    ...
    await context.sdkcall("KinesisVideo", "listStreams", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.StreamInfoList.map(stream => {
            return context.sdkcall("KinesisVideo", "describeStream", {
    ...
    await context.sdkcall("KinesisVideo", "listSignalingChannels", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.ChannelInfoList.map(channel => {
            return context.sdkcall("KinesisVideo", "describeSignalingChannel", {
