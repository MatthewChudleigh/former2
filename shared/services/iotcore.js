...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("Iot", "listThings", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.things.map(thing => {
            if (thing.thingName) {
                return Promise.all([
                    context.sdkcall("Iot", "listThingPrincipals", {
                    ...
                    context.sdkcall("Iot", "describeThing", {
                        thingName: thing.thingName
                    }, true).then((data) => {
                        resources.push({
                            f2id: data.thingArn,
    ...
    await context.sdkcall("Iot", "listPolicies", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.policies.map(policy => {
            return Promise.all([
                context.sdkcall("Iot", "listPolicyPrincipals", {
    ...
    await context.sdkcall("Iot", "listCertificates", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.certificates.map(certificate => {
            return context.sdkcall("Iot", "describeCertificate", {
    ...
    await context.sdkcall("Iot", "listTopicRules", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.rules.map(rule => {
            return context.sdkcall("Iot", "getTopicRule", {
    ...
    await context.sdkcall("Iot", "listProvisioningTemplates", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.templates.map(template => {
            return context.sdkcall("Iot", "describeProvisioningTemplate", {
    ...
    await context.sdkcall("Iot", "listAuthorizers", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.authorizers.map(authorizer => {
            return context.sdkcall("Iot", "describeAuthorizer", {
    ...
    await context.sdkcall("Iot", "listDomainConfigurations", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.domainConfigurations.map(async (domainConfiguration) => {
            return context.sdkcall("Iot", "describeDomainConfiguration", {
                domainConfigurationName: domainConfiguration.domainConfigurationName
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.Configuration.FunctionArn);
    ...
    await context.sdkcall("Iot", "listTopicRuleDestinations", {
        // no params
    }, false).then(async (data) => {
    ...
    await context.sdkcall("IoTWireless", "listDestinations", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.DestinationList.map(async (destination) => {
            return context.sdkcall("IoTWireless", "getDestination", {
                Name: destination.Name
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.Arn);
    ...
    await context.sdkcall("IoTWireless", "listWirelessDevices", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.WirelessDeviceList.map(async (device) => {
            return context.sdkcall("IoTWireless", "getWirelessDevice", {
    ...
    await context.sdkcall("IoTWireless", "listWirelessGateways", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.WirelessGatewayList.map(async (gateway) => {
            return context.sdkcall("IoTWireless", "getWirelessGateway", {
    ...
    await context.sdkcall("IoTWireless", "listDeviceProfiles", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.DeviceProfileList.map(async (profile) => {
            return context.sdkcall("IoTWireless", "getDeviceProfile", {
                Id: profile.Id
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.Arn);
    ...
    await context.sdkcall("IoTWireless", "listServiceProfiles", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.ServiceProfileList.map(async (profile) => {
            return context.sdkcall("IoTWireless", "getServiceProfile", {
                Id: profile.Id
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.Arn);
    ...
    await context.sdkcall("Iot", "describeAccountAuditConfiguration", {
        // no params
    }, false).then(async (data) => {
        if (data.roleArn) {
    ...
    await context.sdkcall("Iot", "listCustomMetrics", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.metricNames.map(async (metricname) => {
            return context.sdkcall("Iot", "describeCustomMetric", {
                metricName: metricname
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.metricArn);
    ...
    await context.sdkcall("Iot", "listDimensions", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.dimensionNames.map(async (dimensionname) => {
            return context.sdkcall("Iot", "describeDimension", {
                name: dimensionname
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.arn);
    ...
    await context.sdkcall("Iot", "listMitigationActions", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.actionIdentifiers.map(async (action) => {
            return context.sdkcall("Iot", "describeMitigationAction", {
                actionName: action.actionName
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.actionArn);
    ...
    await context.sdkcall("Iot", "listScheduledAudits", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.scheduledAudits.map(async (scheduledaudit) => {
            return context.sdkcall("Iot", "describeScheduledAudit", {
                scheduledAuditName: scheduledaudit.scheduledAuditName
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.scheduledAuditArn);
    ...
    await context.sdkcall("Iot", "listSecurityProfiles", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.securityProfileIdentifiers.map(async (securityprofile) => {
            return context.sdkcall("Iot", "describeSecurityProfile", {
                securityProfileName: securityprofile.name
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.securityProfileArn);

                await context.sdkcall("Iot", "listTargetsForSecurityProfile", {
                    securityProfileName: securityprofile.name
    ...
    await context.sdkcall("IoTWireless", "listPartnerAccounts", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.Sidewalk.map(async (sidewalk) => {
            return context.sdkcall("IoTWireless", "getPartnerAccount", {
                PartnerAccountId: sidewalk.AmazonId,
                PartnerType: 'Sidewalk'
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.Sidewalk.Arn);
    ...
    await context.sdkcall("IoTWireless", "listWirelessGatewayTaskDefinitions", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.TaskDefinitions.map(async (taskdefinition) => {
            return context.sdkcall("IoTWireless", "getWirelessGatewayTaskDefinition", {
    ...
    await context.sdkcall("IotDeviceAdvisor", "listSuiteDefinitions", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.suiteDefinitionInformationList.map(async (suitedefinition) => {
            return context.sdkcall("IotDeviceAdvisor", "getSuiteDefinition", {
                suiteDefinitionId: suitedefinition.suiteDefinitionId
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.suiteDefinitionArn);
    ...
    await context.sdkcall("Iot", "listFleetMetrics", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.fleetMetrics.map(async (fleetmetric) => {
            return context.sdkcall("Iot", "describeFleetMetric", {
    ...
    await context.sdkcall("Iot", "listJobTemplates", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.jobTemplates.map(async (jobtemplate) => {
            return context.sdkcall("Iot", "describeJobTemplate", {
                jobTemplateId: jobtemplate.jobTemplateId
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.jobTemplateArn);
    ...
    await context.sdkcall("STS", "getCallerIdentity", {
        ...
        await context.sdkcall("Iot", "getV2LoggingOptions", {
            // no params
        }, false).then(async (data) => {
    ...
    await context.sdkcall("Iot", "listV2LoggingLevels", {
    ...
    await context.sdkcall("IoTWireless", "listFuotaTasks", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.FuotaTaskList.map(async (fuotatask) => {
            return context.sdkcall("IoTWireless", "getFuotaTask", {
                Id: fuotatask.Id
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.Arn);
    ...
    await context.sdkcall("IoTWireless", "listMulticastGroups", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.MulticastGroupList.map(async (multicastgroup) => {
            return context.sdkcall("IoTWireless", "getMulticastGroup", {
    ...
    await context.sdkcall("Iot", "listRoleAliases", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.roleAliases.map(async (rolealias) => {
            return context.sdkcall("Iot", "describeRoleAlias", {
                roleAlias: rolealias
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.roleAliasDescription.roleAliasArn);
    ...
    await context.sdkcall("IoTWireless", "listNetworkAnalyzerConfigurations", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.NetworkAnalyzerConfigurationList.map(async (nac) => {
            return context.sdkcall("IoTWireless", "getNetworkAnalyzerConfiguration", {
    ...
    await context.sdkcall("Iot", "listCACertificates", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.certificates.map(async (certificate) => {
            return context.sdkcall("Iot", "describeCACertificate", {
                certificateId: certificate.certificateId
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.Arn);
