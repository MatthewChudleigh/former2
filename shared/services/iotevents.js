...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("IoTEvents", "listDetectorModels", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.detectorModelSummaries.map(async (detectorModel) => {
            return context.sdkcall("IoTEvents", "describeDetectorModel", {
    ...
    await context.sdkcall("IoTEvents", "listInputs", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.inputSummaries.map(input => {
            return context.sdkcall("IoTEvents", "describeInput", {
    ...
    await context.sdkcall("IoTEvents", "listAlarmModels", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.alarmModelSummaries.map(alarmmodel => {
            return context.sdkcall("IoTEvents", "describeAlarmModel", {
