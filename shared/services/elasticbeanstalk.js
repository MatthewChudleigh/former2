...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("ElasticBeanstalk", "describeApplications", {
        // no params
        ...
        await Promise.all(data.Applications.map(async (application) => {
            ...
            return context.sdkcall("ElasticBeanstalk", "describeEnvironments", {
                ApplicationName: application.ApplicationName
            }, true).then(async (data) => {
                await Promise.all(data.Environments.map(async (environment) => {
                    return context.sdkcall("ElasticBeanstalk", "describeConfigurationSettings", {
                        ApplicationName: application.ApplicationName,
    ...
    await context.sdkcall("ElasticBeanstalk", "describeApplicationVersions", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("ElasticBeanstalk", "describeEnvironments", {
        // no params
    }, true).then(async (data) => {
        data.Environments.forEach(async (environment) => {
            environment['Tags'] = await context.getResourceTags(environment.EnvironmentArn);
