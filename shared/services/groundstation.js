...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("GroundStation", "listMissionProfiles", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.missionProfileList.map(missionProfile => {
            return context.sdkcall("GroundStation", "getMissionProfile", {
    ...
    await context.sdkcall("GroundStation", "listConfigs", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.configList.map(config => {
            return context.sdkcall("GroundStation", "getConfig", {
    ...
    await context.sdkcall("GroundStation", "listDataflowEndpointGroups", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.dataflowEndpointGroupList.map(dataflowEndpointGroup => {
            return context.sdkcall("GroundStation", "getDataflowEndpointGroup", {
