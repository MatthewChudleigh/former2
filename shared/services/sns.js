...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("SNS", "listTopics", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.Topics.map(topic => {
            return Promise.all([
                context.sdkcall("SNS", "getTopicAttributes", {
                    TopicArn: topic.TopicArn
                }, true).then(async (data) => {
                    data['TopicArn'] = topic.TopicArn;
                    await context.sdkcall("SNS", "listTagsForResource", {
                ...
                context.sdkcall("SNS", "listSubscriptionsByTopic", {
                    TopicArn: topic.TopicArn
                }, true).then((data) => {
                    data.Subscriptions.forEach(subscription => {
                        context.sdkcall("SNS", "getSubscriptionAttributes", {
