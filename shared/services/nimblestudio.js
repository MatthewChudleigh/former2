...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("Nimble", "listStudios", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.studios.map(async (studio) => {
            await context.sdkcall("Nimble", "getStudio", {
            ...
            await context.sdkcall("Nimble", "listStudioComponents", {
                studioId: studio.studioId
            }, true).then(async (data) => {
                await Promise.all(data.studioComponents.map(async (studiocomponent) => {
                    return context.sdkcall("Nimble", "getStudioComponent", {
                        studioId: studio.studioId,
            ...
            await context.sdkcall("Nimble", "listStreamingImages", {
                studioId: studio.studioId
            }, true).then(async (data) => {
                await Promise.all(data.streamingImages.map(async (streamingimage) => {
                    return context.sdkcall("Nimble", "getStreamingImage", {
                        studioId: studio.studioId,
            ...
            return context.sdkcall("Nimble", "listLaunchProfiles", {
                studioId: studio.studioId
            }, true).then(async (data) => {
                await Promise.all(data.launchProfiles.map(async (launchprofile) => {
                    return context.sdkcall("Nimble", "getLaunchProfile", {
                        studioId: studio.studioId,
