...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("ForecastService", "listDatasets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.Datasets.map(async (dataset) => {
            return context.sdkcall("ForecastService", "describeDataset", {
    ...
    await context.sdkcall("ForecastService", "listDatasetGroups", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.DatasetGroups.map(async (datasetgroup) => {
            return context.sdkcall("ForecastService", "describeDatasetGroup", {
