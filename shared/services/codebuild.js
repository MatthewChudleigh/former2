...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("CodeBuild", "listProjects", {
        // no params
    }, true).then(async (data) => {
        if (data.projects && data.projects.length) {
            await context.sdkcall("CodeBuild", "batchGetProjects", {
                names: data.projects
            }, true).then((data) => {
    ...
    await context.sdkcall("CodeBuild", "listSourceCredentials", {
        // no params
    }, true).then(async (data) => {
    ...
    await context.sdkcall("CodeBuild", "listReportGroups", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.reportGroups.map(async (reportGroup) => {
            return context.sdkcall("CodeBuild", "batchGetReportGroups", {
                reportGroupArns: [reportGroup]
            }, true).then(async (data) => {
                data.reportGroups[0]['Tags'] = await context.getResourceTags(data.reportGroups[0].arn);
