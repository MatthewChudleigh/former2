...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("CloudFront", "listCloudFrontOriginAccessIdentities", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("CloudFront", "listOriginAccessControls", {
        // no params
    }, false).then((data) => {
    ...
    await context.sdkcall("CloudFront", "listDistributions", {
        // no params
    }, true).then(async (data) => {
        await Promise.all((data.DistributionList.Items || []).map(async (distribution) => {
            return context.sdkcall("CloudFront", "getDistribution", {
                Id: distribution.Id
            }, true).then(async (data) => {
                data.Distribution['Tags'] = await context.getResourceTags(data.Distribution.ARN);
    ...
    await context.sdkcall("CloudFront", "listStreamingDistributions", {
        // no params
    }, true).then(async (data) => {
        (data.StreamingDistributionList.Items || []).forEach(async (distribution) => {
            distribution['Tags'] = await context.getResourceTags(distribution.ARN);
    ...
    await context.sdkcall("CloudFront", "listContinuousDeploymentPolicies", {
        // no params
    }, true).then(async (data) => {
        await Promise.all((data.ContinuousDeploymentPolicyList.Items || []).map(continuousDeploymentPolicy => {
            return context.sdkcall("CloudFront", "getContinuousDeploymentPolicy", {
    ...
    await context.sdkcall("CloudFront", "listCachePolicies", {
        Type: "custom"
    }, true).then(async (data) => {
        await Promise.all((data.CachePolicyList.Items || []).map(cachePolicy => {
            return context.sdkcall("CloudFront", "getCachePolicy", {
    ...
    await context.sdkcall("CloudFront", "listOriginRequestPolicies", {
        Type: "custom"
    }, true).then(async (data) => {
        await Promise.all((data.OriginRequestPolicyList.Items || []).map(originRequestPolicy => {
            return context.sdkcall("CloudFront", "getOriginRequestPolicy", {
    ...
    await context.sdkcall("CloudFront", "listRealtimeLogConfigs", {
        // no params
    }, true).then(async (data) => {
        await Promise.all((data.RealtimeLogConfigs.Items || []).map(realtimeLogConfig => {
            return context.sdkcall("CloudFront", "getRealtimeLogConfig", {
    ...
    await context.sdkcall("CloudFront", "listKeyGroups", {
        // no params
    }, true).then(async (data) => {
        await Promise.all((data.KeyGroupList.Items || []).map(keygroup => {
            return context.sdkcall("CloudFront", "getKeyGroup", {
    ...
    await context.sdkcall("CloudFront", "listPublicKeys", {
        // no params
    }, true).then(async (data) => {
        await Promise.all((data.PublicKeyList.Items || []).map(publickey => {
            return context.sdkcall("CloudFront", "getPublicKey", {
                Id: publickey.Id
            }, true).then((data) => {
                resources.push({
    ...
    await context.sdkcall("CloudFront", "listFunctions", {
        // no params
    }, true).then(async (data) => {
        await Promise.all((data.FunctionList.Items || []).map(async (func) => {
            ...
            return context.sdkcall("CloudFront", "describeFunction", {
                Name: func.Name,
                Stage: func.FunctionMetadata.Stage
            }, true).then(async (data) => {
                await context.sdkcall("CloudFront", "getFunction", {
                    Name: func.Name,
                    Stage: func.FunctionMetadata.Stage
                }, true).then(codedata => {
    ...
    await context.sdkcall("CloudFront", "listResponseHeadersPolicies", {
        // no params
    }, true).then(async (data) => {
        await Promise.all((data.ResponseHeadersPolicyList.Items || []).map(responseheaderpolicy => {
            if (responseheaderpolicy.Type == "custom") {
                return context.sdkcall("CloudFront", "getResponseHeadersPolicyConfig", {
