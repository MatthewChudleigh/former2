...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("ManagedBlockchain", "listNetworks", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.Networks.map(async (network) => {
            return context.sdkcall("ManagedBlockchain", "getNetwork", {
                NetworkId: network.Id
            }, false).then(async (networkdetails) => {
                await context.sdkcall("ManagedBlockchain", "listMembers", {
                    NetworkId: network.Id
                }, false).then(async (data) => {
                    await Promise.all(data.Members.map(async (member) => {
                        return Promise.all([
                            context.sdkcall("ManagedBlockchain", "getMember", {
                                NetworkId: network.Id,
                                MemberId: member.Id
                            }, false).then(async (data) => {
                            ...
                            context.sdkcall("ManagedBlockchain", "listNodes", {
                                NetworkId: network.Id,
                                MemberId: member.Id
                            }, false).then(async (data) => {
                                await Promise.all(data.Nodes.map(async (node) => {
                                    return context.sdkcall("ManagedBlockchain", "getNode", {
                                        NetworkId: network.Id,
                                        MemberId: member.Id,
                                        NodeId: node.Id
                                    }, false).then(async (data) => {
