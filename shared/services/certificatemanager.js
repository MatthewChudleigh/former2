...
async function updateDatatable(context) {
    ...
    await context.sdkcall("ACM", "listCertificates", {
        ...
        await Promise.all(data.CertificateSummaryList.map(async (certificate) => {
            return context.sdkcall("ACM", "describeCertificate", {
                CertificateArn: certificate.CertificateArn
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags(data.Certificate.CertificateArn);
    ...
    await context.sdkcall("ACMPCA", "listCertificateAuthorities", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.CertificateAuthorities.map(async (certificateAuthority) => {
            await context.sdkcall("ACMPCA", "describeCertificateAuthority", {
                ...
                if (data.CertificateAuthority.Type == "ACTIVE" || data.CertificateAuthority.Type == "DISABLED") {
                    await context.sdkcall("ACMPCA", "getCertificateAuthorityCertificate", {
                        CertificateAuthorityArn: certificateAuthority.Arn
            ...
            return context.sdkcall("ACMPCA", "listPermissions", {
                CertificateAuthorityArn: certificateAuthority.Arn
            }, true).then(async (data) => {
    ...
    await context.sdkcall("ACM", "getAccountConfiguration", {
        // no params
    }, true).then(async (data) => {
