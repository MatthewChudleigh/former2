...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("ES", "listDomainNames", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.DomainNames.map(async (domainName) => {
            return context.sdkcall("ES", "describeElasticsearchDomains", {
                DomainNames: [domainName.DomainName]
            }, true).then(async (data) => {
                data.DomainStatusList[0]['Tags'] = await context.getResourceTags(data.DomainStatusList[0].ARN);
