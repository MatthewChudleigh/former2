...
async function updateDatatable(context) {
    ...
    await Promise.all([
        context.sdkcall("IAM", "listGroups", {
            // no params
        }, true).then(async (data) => {
            await Promise.all(data.Groups.map(async (group) => {
                await context.sdkcall("IAM", "listAttachedGroupPolicies", {
                ...
                return context.sdkcall("IAM", "listGroupPolicies", {
                    GroupName: group.GroupName
                }, true).then(async (data) => {
                    await Promise.all(data.PolicyNames.map(policyname => {
                        return context.sdkcall("IAM", "getGroupPolicy", {
            ...
            await context.sdkcall("IAM", "listUsers", {
                ...
                await Promise.all(data.Users.map(async (user) => {
                    ...
                    await context.sdkcall("IAM", "listAccessKeys", {
                        UserName: user.UserName
                    }, true).then((data) => {
                    ...
                    return context.sdkcall("IAM", "listUserPolicies", {
                        UserName: user.UserName
                    }, true).then(async (data) => {
                        await Promise.all(data.PolicyNames.map(policyname => {
                            return context.sdkcall("IAM", "getUserPolicy", {
                                PolicyName: policyname,
                                UserName: user.UserName
                            }, true).then(async (policydata) => {
                                resources.push({
        ...
        context.sdkcall("IAM", "listRoles", {
            // no params
        }, true).then(async (data) => {
            await Promise.all(data.Roles.map(async (role) => {
                await context.sdkcall("IAM", "getRole", {
                ...
                if (role.Path.startsWith("/aws-service-role/")) {
                    ...
                    await context.sdkcall("IAM", "listAttachedRolePolicies", {
                        RoleName: role.RoleName
                    }, true).then((attachedpolicies) => {
                ...
                return context.sdkcall("IAM", "listRolePolicies", {
                    RoleName: role.RoleName
                }, true).then(async (data) => {
                    await Promise.all(data.PolicyNames.map(policyname => {
                        return context.sdkcall("IAM", "getRolePolicy", {
                            PolicyName: policyname,
                            RoleName: role.RoleName
                        }, true).then(async (policydata) => {
                            resources.push({
    ...
    await context.sdkcall("IAM", "listInstanceProfiles", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("AccessAnalyzer", "listAnalyzers", {
        type: 'ACCOUNT'
    }, true).then(async (data) => {
        data.analyzers.forEach(async (analyzer) => {
            await context.sdkcall("AccessAnalyzer", "getAnalyzer", {
                analyzerName: analyzer.name
            }, true).then(async (data) => {
                await context.sdkcall("AccessAnalyzer", "listArchiveRules", {
                    analyzerName: analyzer.name
    ...
    await context.sdkcall("IAM", "listVirtualMFADevices", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("IAM", "listVirtualMFADevices", {
        ...
        await Promise.all(data.ServerCertificateMetadataList.map(serverCertificate => {
            return context.sdkcall("IAM", "getServerCertificate", {
    ...
    await context.sdkcall("IAM", "listSAMLProviders", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.SAMLProviderList.map(SAMLProvider => {
            return context.sdkcall("IAM", "getSAMLProvider", {
    ...
    await context.sdkcall("IAM", "listOpenIDConnectProviders", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.OpenIDConnectProviderList.map(provider => {
            return context.sdkcall("IAM", "getOpenIDConnectProvider", {
