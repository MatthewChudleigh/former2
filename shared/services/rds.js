...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("RDS", "describeDBClusters", {
        Filters: [{
            Name: 'engine',
        ...
        data.DBClusters.forEach(async (dbCluster) => {
            dbCluster['Tags'] = await context.getResourceTags(dbCluster.DBClusterArn);
    ...
    await context.sdkcall("RDS", "describeDBInstances", {
        // no params
    }, true).then(async (data) => {
    ...
    await context.sdkcall("RDS", "describeGlobalClusters", {
        // no params
    }, true).then(async (data) => {
    ...
    await context.sdkcall("RDS", "describeDBSubnetGroups", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("RDS", "describeDBParameterGroups", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.DBParameterGroups.map(parameterGroup => {
            return context.sdkcall("RDS", "describeDBParameters", {
    ...
    await context.sdkcall("RDS", "describeDBClusterParameterGroups", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.DBClusterParameterGroups.map(parameterGroup => {
            return context.sdkcall("RDS", "describeDBClusterParameters", {
    ...
    await context.sdkcall("RDS", "describeOptionGroups", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("RDS", "describeDBSecurityGroups", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("RDS", "describeEventSubscriptions", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("RDS", "describeDBProxies", {
        // no params
    }, true).then(async (data) => {
        if (data.DBProxies) {
            await Promise.all(data.DBProxies.map(async (proxy) => {
                ...
                return context.sdkcall("RDS", "describeDBProxyTargetGroups", {
                    DBProxyName: proxy.DBProxyName
                }, true).then(async (targetgroups) => {
                    targetgroups.TargetGroups.forEach(async (tg) => {
                        await context.sdkcall("RDS", "describeDBProxyTargets", {
                            DBProxyName: proxy.DBProxyName,
    ...
    await context.sdkcall("RDS", "describeDBProxyEndpoints", {
        // no params
