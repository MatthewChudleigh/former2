...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("Transfer", "listServers", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.Servers.map(async (server) => {
            await context.sdkcall("Transfer", "listUsers", {
                ServerId: server.ServerId
            }, true).then(async (data) => {
                await Promise.all(data.Users.map(user => {
                    return context.sdkcall("Transfer", "describeUser", {
                        ServerId: server.ServerId,
            ...
            if (server.IdentityProviderType != "SERVICE_MANAGED") {
                await context.sdkcall("Transfer", "listAccesses", {
                    ServerId: server.ServerId
                }, true).then(async (data) => {
                    await Promise.all(data.Accesses.map(access => {
                        return context.sdkcall("Transfer", "describeAccess", {
                            ServerId: server.ServerId,
            ...
            return context.sdkcall("Transfer", "describeServer", {
                ServerId: server.ServerId
            }, true).then(async (data) => {
                resources.push({
        ...
        await context.sdkcall("Transfer", "listWorkflows", {
            // no params
        }, true).then(async (data) => {
            await Promise.all(data.Workflows.map(async (workflow) => {
                return context.sdkcall("Transfer", "describeWorkflow", {
