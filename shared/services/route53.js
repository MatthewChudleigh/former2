...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("Route53", "listHostedZones", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.HostedZones.map(async (hostedZone) => {
            await context.sdkcall("Route53", "getHostedZone", {
                Id: hostedZone.Id.split("/").pop()
            }, true).then(async (data) => {
                data['Tags'] = await context.getResourceTags("arn:aws:route53:::hostedzone/" + hostedZone.Id.split("/").pop());
            ...
            await context.sdkcall("Route53", "listResourceRecordSets", {
                HostedZoneId: hostedZone.Id.split("/").pop()
            }, true).then(async (data) => {
            ...
            return context.sdkcall("Route53", "getDNSSEC", {
                HostedZoneId: hostedZone.Id.split("/").pop()
    ...
    await context.sdkcall("Route53", "listHealthChecks", {
        // no params
    }, true).then((data) => {
    ...
    await context.sdkcall("Route53Resolver", "listResolverEndpoints", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.ResolverEndpoints.map(async (resolverEndpoint) => {
            return context.sdkcall("Route53Resolver", "getResolverEndpoint", {
                ResolverEndpointId: resolverEndpoint.Id
            }, true).then(async (data) => {
                await context.sdkcall("Route53Resolver", "listResolverEndpointIpAddresses", {
                    ResolverEndpointId: resolverEndpoint.Id
                ...
                await context.sdkcall("Route53Resolver", "getResolverDnssecConfig", {
                ...
                await context.sdkcall("Route53Resolver", "getResolverConfig", {
                    ResourceId: data.ResolverEndpoint.HostVPCId
                }, true).then((data) => {
                    resources.push({
    ...
    await context.sdkcall("Route53Resolver", "listResolverRules", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.ResolverRules.map(resolverRule => {
            return context.sdkcall("Route53Resolver", "getResolverRule", {
    ...
    await context.sdkcall("Route53Resolver", "listResolverRuleAssociations", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.ResolverRuleAssociations.map(resolverRuleAssociation => {
            return context.sdkcall("Route53Resolver", "getResolverRuleAssociation", {
    ...
    await context.sdkcall("Route53Resolver", "listResolverQueryLogConfigs", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.ResolverQueryLogConfigs.map(async (resolverQueryLogConfig) => {
            return context.sdkcall("Route53Resolver", "getResolverQueryLogConfig", {
    ...
    await context.sdkcall("Route53Resolver", "listResolverQueryLogConfigAssociations", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.ResolverQueryLogConfigAssociations.map(async (resolverQueryLogConfigAssociation) => {
            return context.sdkcall("Route53Resolver", "getResolverQueryLogConfigAssociation", {
    ...
    await context.sdkcall("Route53Resolver", "listFirewallDomainLists", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.FirewallDomainLists.map(async (firewalldomainlist) => {
            return context.sdkcall("Route53Resolver", "getFirewallDomainList", {
                FirewallDomainListId: firewalldomainlist.Id
            }, true).then(async (data) => {
                await context.sdkcall("Route53Resolver", "listFirewallDomains", {
                    FirewallDomainListId: firewalldomainlist.Id
    ...
    await context.sdkcall("Route53Resolver", "listFirewallRuleGroups", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.FirewallRuleGroups.map(async (firewallrulegroup) => {
            await context.sdkcall("Route53Resolver", "getFirewallRuleGroup", {
                FirewallRuleGroupId: firewallrulegroup.Id
            }, true).then(async (data) => {
                await context.sdkcall("Route53Resolver", "listFirewallRules", {
                    FirewallRuleGroupId: firewallrulegroup.Id
            ...
            return context.sdkcall("Route53Resolver", "listFirewallRuleGroupAssociations", {
                FirewallRuleGroupId: firewallrulegroup.Id
            }, true).then(async (data) => {
                await Promise.all(data.FirewallRuleGroupAssociations.map(async (firewallrulegroupassociation) => {
                    return context.sdkcall("Route53Resolver", "getFirewallRuleGroupAssociation", {
    ...
    await context.sdkcall("Route53RecoveryControlConfig", "listClusters", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.Clusters.map(async (cluster) => {
            await context.sdkcall("Route53RecoveryControlConfig", "describeCluster", {
            ...
            return context.sdkcall("Route53RecoveryControlConfig", "listControlPanels", {
                ClusterArn: cluster.ClusterArn
            }, true).then(async (data) => {
                await Promise.all(data.ControlPanels.map(async (controlpanel) => {
                    await context.sdkcall("Route53RecoveryControlConfig", "describeControlPanel", {
                    ...
                    await context.sdkcall("Route53RecoveryControlConfig", "listRoutingControls", {
                        ControlPanelArn: controlpanel.ControlPanelArn
                    }, true).then(async (data) => {
                        await Promise.all(data.RoutingControls.map(async (routingcontrol) => {
                            return context.sdkcall("Route53RecoveryControlConfig", "describeRoutingControl", {
                    ...
                    return context.sdkcall("Route53RecoveryControlConfig", "listSafetyRules", {
                        ControlPanelArn: controlpanel.ControlPanelArn
                    }, true).then(async (data) => {
                        await Promise.all(data.SafetyRules.map(async (safetyrule) => {
                            return context.sdkcall("Route53RecoveryControlConfig", "describeSafetyRule", {
    ...
    await context.sdkcall("Route53RecoveryReadiness", "listRecoveryGroups", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.RecoveryGroups.map(async (recoverygroup) => {
            return context.sdkcall("Route53RecoveryReadiness", "getRecoveryGroup", {
    ...
    await context.sdkcall("Route53RecoveryReadiness", "listCells", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.Cells.map(async (cell) => {
            return context.sdkcall("Route53RecoveryReadiness", "getCell", {
    ...
    await context.sdkcall("Route53RecoveryReadiness", "listResourceSets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.ResourceSets.map(async (resourceset) => {
            return context.sdkcall("Route53RecoveryReadiness", "getResourceSet", {
    ...
    await context.sdkcall("Route53RecoveryReadiness", "listReadinessChecks", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.ReadinessChecks.map(async (readinesscheck) => {
            return context.sdkcall("Route53RecoveryReadiness", "getReadinessCheck", {
    ...
    await context.sdkcall("Route53RecoveryReadiness", "listCidrCollections", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.CidrCollections.map(async (cidrcollection) => {
            return context.sdkcall("Route53RecoveryReadiness", "listCidrBlocks", {
