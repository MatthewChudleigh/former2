...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("CognitoIdentity", "listIdentityPools", {
        MaxResults: 60 // WTF, required!
    }, true).then(async (data) => {
        await Promise.all(data.IdentityPools.map(identityPool => {
            return Promise.all([
                context.sdkcall("CognitoIdentity", "getIdentityPoolRoles", {
                ...
                context.sdkcall("CognitoIdentity", "describeIdentityPool", {
                    IdentityPoolId: identityPool.IdentityPoolId
                }, true).then((data) => {
                    resources.push({
                        f2id: data.IdentityPoolId,
    ...
    await context.sdkcall("CognitoIdentityServiceProvider", "listUserPools", {
        MaxResults: 60 // WTF? required!
    }, true).then(async (data) => {
        await Promise.all(data.UserPools.map(async (userPool) => {
            ...
            await Promise.all([
                context.sdkcall("CognitoIdentityServiceProvider", "listResourceServers", {
                    UserPoolId: userPool.Id,
                    MaxResults: 50 // WTF? required!
                }, true).then(async (data) => {
                    await Promise.all(data.ResourceServers.map(resourceServer => {
                        return context.sdkcall("CognitoIdentityServiceProvider", "describeResourceServer", {
                ...
                context.sdkcall("CognitoIdentityServiceProvider", "listIdentityProviders", {
                    UserPoolId: userPool.Id
                }, true).then(async (data) => {
                    await Promise.all(data.Providers.map(provider => {
                        return context.sdkcall("CognitoIdentityServiceProvider", "describeIdentityProvider", {
                            UserPoolId: userPool.Id,
                ...
                context.sdkcall("CognitoIdentityServiceProvider", "describeRiskConfiguration", {
                    UserPoolId: userPool.Id
                }, false).then((data) => {
                    resources.push({
                ...
                context.sdkcall("CognitoIdentityServiceProvider", "getUICustomization", {
                    UserPoolId: userPool.Id
                }, false).then((data) => {
                    resources.push({
                ...
                context.sdkcall("CognitoIdentityServiceProvider", "listUserPoolClients", {
                    UserPoolId: userPool.Id
                }, true).then(async (data) => {
                    await Promise.all(data.UserPoolClients.map(userPoolClient => {
                        return context.sdkcall("CognitoIdentityServiceProvider", "describeUserPoolClient", {
                            UserPoolId: userPool.Id,
                ...
                context.sdkcall("CognitoIdentityServiceProvider", "listUsers", {
                    ...
                    await Promise.all(data.Users.map(async (user) => {
                        await context.sdkcall("CognitoIdentityServiceProvider", "adminListGroupsForUser", {
                            UserPoolId: userPool.Id,
                        ...
                        return context.sdkcall("CognitoIdentityServiceProvider", "adminGetUser", {
                            UserPoolId: userPool.Id,
                            Username: user.Username
                        }, true).then((data) => {
                ...
                context.sdkcall("CognitoIdentityServiceProvider", "listGroups", {
                    UserPoolId: userPool.Id
                }, true).then(async (data) => {
                    await Promise.all(data.Groups.map(group => {
                        return context.sdkcall("CognitoIdentityServiceProvider", "getGroup", {
                            UserPoolId: userPool.Id,
                            GroupName: group.GroupName
                        }, true).then((data) => {
                            resources.push({
            ...
            return context.sdkcall("CognitoIdentityServiceProvider", "describeUserPool", {
                UserPoolId: userPool.Id
            }, true).then(async (data) => {
                ...
                await context.sdkcall("CognitoIdentityServiceProvider", "describeUserPoolDomain", {
