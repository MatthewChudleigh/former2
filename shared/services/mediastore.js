...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("MediaStore", "listContainers", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.Containers.map(container => {
            return Promise.all([
                context.sdkcall("MediaStore", "describeContainer", {
                    ContainerName: container.Name
                }, true).then(async (containerdata) => {
                    await Promise.all([
                        context.sdkcall("MediaStore", "getCorsPolicy", {
                            ContainerName: container.Name
                        }, false).then((data) => {
                            containerdata['CorsPolicy'] = data.CorsPolicy;
                        }).catch(() => { }),
                        context.sdkcall("MediaStore", "getLifecyclePolicy", {
                            ContainerName: container.Name
                        }, false).then((data) => {
                            containerdata['LifecyclePolicy'] = data.LifecyclePolicy;
                        }).catch(() => { }),
                        context.sdkcall("MediaStore", "getContainerPolicy", {
                            ContainerName: container.Name
                        }, false).then((data) => {
                            containerdata['Policy'] = data.Policy;
                        }).catch(() => { }),
                        context.sdkcall("MediaStore", "getMetricPolicy", {
                            ContainerName: container.Name
                        }, false).then((data) => {
