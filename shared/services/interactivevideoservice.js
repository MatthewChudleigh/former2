...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("IVS", "listChannels", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.channels.map(async (channel) => {
            await context.sdkcall("IVS", "getChannel", {
                arn: channel.arn
            }, true).then((data) => {
                resources.push({
            ...
            return context.sdkcall("IVS", "listStreamKeys", {
                channelArn: channel.arn
            }, true).then(async (data) => {
                await Promise.all(data.streamKeys.map(async (streamkey) => {
                    return context.sdkcall("IVS", "getStreamKey", {
                        arn: streamkey.arn
                    }, true).then((data) => {
                        resources.push({
    ...
    await context.sdkcall("IVS", "listPlaybackKeyPairs", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.keyPairs.map(async (keypair) => {
            return context.sdkcall("IVS", "getPlaybackKeyPair", {
                arn: keypair.arn
            }, true).then((data) => {
                resources.push({
    ...
    await context.sdkcall("IVS", "listRecordingConfigurations", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.recordingConfigurations.map(async (recordingConfiguration) => {
            return context.sdkcall("IVS", "getRecordingConfiguration", {
