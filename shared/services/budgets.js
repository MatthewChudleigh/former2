...
async function updateDatatable(context) {
    ...
    await context.sdkcall("STS", "getCallerIdentity", {
        ...
        await context.sdkcall("Budgets", "describeBudgets", {
            AccountId: accountId
        }, true).then(async (data) => {
            if (data.Budgets) {
                await Promise.all(data.Budgets.map(async (budget) => {
                    await context.sdkcall("Budgets", "describeNotificationsForBudget", {
                        AccountId: accountId,
                    ...
                    return context.sdkcall("Budgets", "describeBudgetActionsForBudget", {
                        AccountId: accountId,
                        BudgetName: budget.BudgetName
                    }, true).then(async (data) => {
