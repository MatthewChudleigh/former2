...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("MigrationHubRefactorSpaces", "listEnvironments", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.EnvironmentSummaryList.map(async (environment) => {
            await context.sdkcall("MigrationHubRefactorSpaces", "getEnvironment", {
            ...
            return context.sdkcall("MigrationHubRefactorSpaces", "listApplications", {
                EnvironmentIdentifier: environment.EnvironmentId
            }, true).then(async (data) => {
                await Promise.all(data.ApplicationSummaryList.map(async (application) => {
                    await context.sdkcall("MigrationHubRefactorSpaces", "getApplication", {
                        EnvironmentIdentifier: environment.EnvironmentId,
                    ...
                    await context.sdkcall("MigrationHubRefactorSpaces", "listRoutes", {
                        EnvironmentIdentifier: environment.EnvironmentId,
                        ApplicationIdentifier: application.ApplicationId
                    }, true).then(async (data) => {
                        await Promise.all(data.RouteSummaryList.map(async (route) => {
                            return context.sdkcall("MigrationHubRefactorSpaces", "getRoute", {
                                EnvironmentIdentifier: environment.EnvironmentId,
                                ApplicationIdentifier: application.ApplicationId,
                    ...
                    return context.sdkcall("MigrationHubRefactorSpaces", "listServices", {
                        EnvironmentIdentifier: environment.EnvironmentId,
                        ApplicationIdentifier: application.ApplicationId
                    }, true).then(async (data) => {
                        await Promise.all(data.ServiceSummaryList.map(async (service) => {
                            return context.sdkcall("MigrationHubRefactorSpaces", "getService", {
                                EnvironmentIdentifier: environment.EnvironmentId,
                                ApplicationIdentifier: application.ApplicationId,
