...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("EKS", "listClusters", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.clusters.map(async (cluster) => {
            return Promise.all([
                context.sdkcall("EKS", "describeCluster", {
                    name: cluster
                }, true).then((data) => {
                    resources.push({
                ...
                context.sdkcall("EKS", "listNodegroups", {
                    clusterName: cluster
                }, true).then(async (data) => {
                    await Promise.all(data.nodegroups.map(async (nodegroup) => {
                        return context.sdkcall("EKS", "describeNodegroup", {
                            clusterName: cluster,
                ...
                context.sdkcall("EKS", "listFargateProfiles", {
                    clusterName: cluster
                }, true).then(async (data) => {
                    await Promise.all(data.fargateProfileNames.map(async (fargateProfileName) => {
                        return context.sdkcall("EKS", "describeFargateProfile", {
                            clusterName: cluster,
                ...
                context.sdkcall("EKS", "listAddons", {
                    clusterName: cluster
                }, true).then(async (data) => {
                    await Promise.all(data.addons.map(async (addon) => {
                        return context.sdkcall("EKS", "describeAddon", {
                            clusterName: cluster,
                            addonName: addon
                        }, true).then((data) => {
                            resources.push({
                ...
                context.sdkcall("EKS", "listIdentityProviderConfigs", {
                    clusterName: cluster
                }, true).then(async (data) => {
                    await Promise.all(data.identityProviderConfigs.map(async (identityProviderConfig) => {
                        return context.sdkcall("EKS", "describeIdentityProviderConfig", {
                            clusterName: cluster,
