...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("Kafka", "listClusters", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.ClusterInfoList.map(async (cluster) => {
            await context.sdkcall("Kafka", "describeCluster", {
            ...
            return context.sdkcall("Kafka", "listScramSecrets", {
                ClusterArn: cluster.ClusterArn
            }, true).then(async (data) => {
                resources.push({
    ...
    await context.sdkcall("Kafka", "listClustersV2", {
        ClusterTypeFilter: "SERVERLESS"
    }, false).then(async (data) => {
        await Promise.all(data.ClusterInfoList.map(async (cluster) => {
            return context.sdkcall("Kafka", "describeClusterV2", {
                ClusterArn: cluster.ClusterArn
            }, true).then(async (data) => {
                resources.push({
                    f2id: data.ClusterInfo.ClusterArn,
    ...
    await context.sdkcall("KafkaConnect", "listConnectors", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.connectors.map(connector => {
            return context.sdkcall("KafkaConnect", "describeConnector", {
    ...
    await context.sdkcall("KafkaConnect", "listConfigurations", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.Configurations.map(configuration => {
            return context.sdkcall("KafkaConnect", "describeConfiguration", {
                Arn: configuration.Arn
            }, true).then(async (data) => {
                await context.sdkcall("KafkaConnect", "describeConfigurationRevision", {
                    Arn: configuration.Arn,
