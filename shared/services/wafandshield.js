...
async function updateDatatable(context) {
    ...
    if (region == "us-east-1") {
        await context.sdkcall("WAFV2", "listWebACLs", {
            ...
            await Promise.all(data.WebACLs.map(webAcl => {
                return context.sdkcall("WAFV2", "getWebACL", {
                    Scope: "CLOUDFRONT",
                    Id: webAcl.Id,
                    Name: webAcl.Name
                }, true).then((data) => {
    ...
    await context.sdkcall("WAFV2", "listWebACLs", {
        ...
        await Promise.all(data.WebACLs.map(async (webAcl) => {
            await context.sdkcall("WAFV2", "listResourcesForWebACL", {
            ...
            await context.sdkcall("WAFV2", "getWebACL", {
                Scope: "REGIONAL",
                Id: webAcl.Id,
                Name: webAcl.Name
            }, true).then((data) => {
            ...
            return context.sdkcall("WAFV2", "getLoggingConfiguration", {
    ...
    if (region == "us-east-1") {
        await context.sdkcall("WAFV2", "listRuleGroups", {
            ...
            await Promise.all(data.RuleGroups.map(ruleGroup => {
                return context.sdkcall("WAFV2", "getRuleGroup", {
                    Scope: "CLOUDFRONT",
                    Id: ruleGroup.Id,
                    Name: ruleGroup.Name
                }, true).then((data) => {
    ...
    if (region == "us-east-1") {
        await context.sdkcall("WAFV2", "listIPSets", {
            ...
            await Promise.all(data.IPSets.map(ipSet => {
                return context.sdkcall("WAFV2", "getIPSet", {
                    Scope: "CLOUDFRONT",
                    Id: ipSet.Id,
                    Name: ipSet.Name
                }, true).then((data) => {
    ...
    if (region == "us-east-1") {
        await context.sdkcall("WAFV2", "listRegexPatternSets", {
            ...
            await Promise.all(data.RegexPatternSets.map(regexPatternSet => {
                return context.sdkcall("WAFV2", "getRegexPatternSet", {
                    Scope: "CLOUDFRONT",
    ...
    await context.sdkcall("WAF", "listWebACLs", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.WebACLs.map(webAcl => {
            return context.sdkcall("WAF", "getWebACL", {
    ...
    await context.sdkcall("WAF", "listRules", {
        // no params
    }, false).then(async (data) => {
        if (data.Rules) {
            await Promise.all(data.Rules.map(rule => {
                return context.sdkcall("WAF", "getRule", {
                    RuleId: rule.RuleId
                }, true).then((data) => {
                    resources.push({
    ...
    await context.sdkcall("WAF", "listXssMatchSets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.XssMatchSets.map(xssMatchSet => {
            return context.sdkcall("WAF", "getXssMatchSet", {
    ...
    await context.sdkcall("WAF", "listIPSets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.IPSets.map(ipSet => {
            return context.sdkcall("WAF", "getIPSet", {
    ...
    await context.sdkcall("WAF", "listSizeConstraintSets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.SizeConstraintSets.map(sizeConstraintSet => {
            return context.sdkcall("WAF", "getSizeConstraintSet", {
    ...
    await context.sdkcall("WAF", "listSqlInjectionMatchSets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.SqlInjectionMatchSets.map(sqlInjectionMatchSet => {
            return context.sdkcall("WAF", "getSqlInjectionMatchSet", {
    ...
    await context.sdkcall("WAF", "listByteMatchSets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.ByteMatchSets.map(byteMatchSet => {
            return context.sdkcall("WAF", "getByteMatchSet", {
    ...
    await context.sdkcall("WAFRegional", "listWebACLs", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.WebACLs.map(webAcl => {
            return Promise.all([
                context.sdkcall("WAFRegional", "getWebACL", {
                    WebACLId: webAcl.WebACLId
                }, true).then((data) => {
                    resources.push({
                        f2id: data.WebACL.WebACLId,
                ...
                context.sdkcall("WAFRegional", "listResourcesForWebACL", {
                    WebACLId: webAcl.WebACLId
                }, true).then((data) => {
                    data.ResourceArns.forEach(resourceArn => {
                        resources.push({
    ...
    await context.sdkcall("WAFRegional", "listRules", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.Rules.map(rule => {
            return context.sdkcall("WAFRegional", "getRule", {
                RuleId: rule.RuleId
            }, true).then((data) => {
                resources.push({
                    f2id: data.Rule.RuleId,
    ...
    await context.sdkcall("WAFRegional", "listXssMatchSets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.XssMatchSets.map(xssMatchSet => {
            return context.sdkcall("WAFRegional", "getXssMatchSet", {
                XssMatchSetId: xssMatchSet.XssMatchSetId
            }, true).then((data) => {
                resources.push({
                    f2id: data.XssMatchSet.XssMatchSetId,
    ...
    await context.sdkcall("WAFRegional", "listIPSets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.IPSets.map(ipSet => {
            return context.sdkcall("WAFRegional", "getIPSet", {
                IPSetId: ipSet.IPSetId
            }, true).then((data) => {
                resources.push({
                    f2id: data.IPSet.IPSetId,
    ...
    await context.sdkcall("WAFRegional", "listSizeConstraintSets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.SizeConstraintSets.map(sizeConstraintSet => {
            return context.sdkcall("WAFRegional", "getSizeConstraintSet", {
                SizeConstraintSetId: sizeConstraintSet.SizeConstraintSetId
            }, true).then((data) => {
                resources.push({
                    f2id: data.SizeConstraintSet.SizeConstraintSetId,
    ...
    await context.sdkcall("WAFRegional", "listSqlInjectionMatchSets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.SqlInjectionMatchSets.map(sqlInjectionMatchSet => {
            return context.sdkcall("WAFRegional", "getSqlInjectionMatchSet", {
                SqlInjectionMatchSetId: sqlInjectionMatchSet.SqlInjectionMatchSetId
            }, true).then((data) => {
                resources.push({
                    f2id: data.SqlInjectionMatchSet.SqlInjectionMatchSetId,
    ...
    await context.sdkcall("WAFRegional", "listByteMatchSets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.ByteMatchSets.map(byteMatchSet => {
            return context.sdkcall("WAFRegional", "getByteMatchSet", {
                ByteMatchSetId: byteMatchSet.ByteMatchSetId
            }, true).then((data) => {
                resources.push({
                    f2id: data.ByteMatchSet.ByteMatchSetId,
    ...
    await context.sdkcall("WAFRegional", "listGeoMatchSets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.GeoMatchSets.map(geoMatchSet => {
            return context.sdkcall("WAFRegional", "getGeoMatchSet", {
    ...
    await context.sdkcall("WAFRegional", "listRegexPatternSets", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.RegexPatternSets.map(regexPatternSet => {
            return context.sdkcall("WAFRegional", "getRegexPatternSet", {
    ...
    await context.sdkcall("WAFRegional", "listRateBasedRules", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.Rules.map(rule => {
            return context.sdkcall("WAFRegional", "getRateBasedRule", {
                RuleId: rule.RuleId
            }, true).then((data) => {
                resources.push({
    ...
    await context.sdkcall("FMS", "listPolicies", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.PolicyList.map(policy => {
            return context.sdkcall("FMS", "getPolicy", {
    ...
    await context.sdkcall("FMS", "getNotificationChannel", {
        // no params
    }, false).then(async (data) => {
        resources.push({
