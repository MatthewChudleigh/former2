...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("ECS", "listTaskDefinitions", {
        sort: "DESC"
        ...
        await Promise.all(taskDefinitionArns.map(taskDefinitionArn => {
            return context.sdkcall("ECS", "describeTaskDefinition", {
    ...
    await context.sdkcall("ECS", "listClusters", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.clusterArns.map(clusterArn => {
            return Promise.all([
                context.sdkcall("ECS", "describeClusters", {
                    clusters: [clusterArn]
                }, true).then((data) => {
                    resources.push({
                ...
                context.sdkcall("ECS", "listServices", {
                    cluster: clusterArn
                }, true).then(async (data) => {
                    await Promise.all(data.serviceArns.map(serviceArn => {
                        return context.sdkcall("ECS", "describeServices", {
                            services: [serviceArn],
                            cluster: clusterArn,
                            include: ["TAGS"]
                        }, true).then(async (data) => {
                            if (data.services[0]) {
                                ...
                                if (data.services[0].taskSets && data.services[0].taskSets.length) {
                                    await context.sdkcall("ECS", "describeTaskSets", {
                                        cluster: clusterArn,
                                        service: serviceArn,
    ...
    await context.sdkcall("ECS", "describeCapacityProviders", {
        include: ["TAGS"]
    }, true).then(async (data) => {
