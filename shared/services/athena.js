...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("Athena", "listNamedQueries", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.NamedQueryIds.map(namedQuery => {
            return context.sdkcall("Athena", "getNamedQuery", {
    ...
    await context.sdkcall("Athena", "listWorkGroups", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.WorkGroups.map(async (workgroup) => {
            await context.sdkcall("Athena", "getWorkGroup", {
                WorkGroup: workgroup.Name
            }, true).then(async (data) => {
                if (
                    data.WorkGroup.Name != "primary"
                ) {
                    data.WorkGroup['Tags'] = await context.getResourceTags("arn:aws:athena:region:account:workgroup/" + data.WorkGroup.Name);
            ...
            return context.sdkcall("Athena", "listPreparedStatements", {
                WorkGroup: workgroup.Name
            }, true).then(async (data) => {
                await Promise.all(data.PreparedStatements.map(async (preparedstatement) => {
                    return context.sdkcall("Athena", "getPreparedStatement", {
                        WorkGroup: workgroup.Name,
    ...
    await context.sdkcall("Athena", "listDataCatalogs", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.DataCatalogsSummary.map(async (datacatalog) => {
            return context.sdkcall("Athena", "getDataCatalog", {
                Name: datacatalog.CatalogName
            }, false).then(async (data) => {
                data.DataCatalog['Tags'] = await context.getResourceTags("arn:aws:athena:region:account:datacatalog/" + data.DataCatalog.Name);
