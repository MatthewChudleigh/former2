...
async function updateDatatable(context) {
    ...
    await context.sdkcall("RoboMaker", "listFleets", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.fleetDetails.map(fleet => {
            return context.sdkcall("RoboMaker", "describeFleet", {
                fleet: fleet.arn
            }, true).then((data) => {
                resources.push({
                    f2id: data.arn,
    ...
    await context.sdkcall("RoboMaker", "listRobots", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.robots.map(robot => {
            return context.sdkcall("RoboMaker", "describeRobot", {
                robot: robot.arn
            }, true).then((data) => {
                resources.push({
                    f2id: data.arn,
    ...
    */
    ...
    await context.sdkcall("RoboMaker", "listRobotApplications", {
        versionQualifier: "ALL"
    }, true).then(async (data) => {
        await Promise.all(data.robotApplicationSummaries.map(robotApplication => {
            return context.sdkcall("RoboMaker", "describeRobotApplication", {
    ...
    await context.sdkcall("RoboMaker", "listSimulationApplications", {
        versionQualifier: "ALL"
    }, true).then(async (data) => {
        await Promise.all(data.simulationApplicationSummaries.map(simulationApplication => {
            return context.sdkcall("RoboMaker", "describeSimulationApplication", {
