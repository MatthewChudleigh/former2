...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("CodeDeploy", "listApplications", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.applications.map(application => {
            return Promise.all([
                context.sdkcall("CodeDeploy", "listDeploymentGroups", {
                    applicationName: application
                }, true).then((data) => {
                    data.deploymentGroups.forEach(deploymentGroup => {
                        context.sdkcall("CodeDeploy", "getDeploymentGroup", {
                            applicationName: application,
                ...
                context.sdkcall("CodeDeploy", "getApplication", {
                    applicationName: application
                }, true).then((data) => {
                    resources.push({
    ...
    await context.sdkcall("CodeDeploy", "listDeploymentConfigs", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.deploymentConfigsList.map(deploymentConfiguration => {
            return context.sdkcall("CodeDeploy", "getDeploymentConfig", {
