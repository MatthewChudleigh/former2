...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("KMS", "listKeys", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.Keys.map(key => {
            return context.sdkcall("KMS", "describeKey", {
                KeyId: key.KeyId
            }, true).then(async (keydata) => {
                if (keydata.KeyMetadata.KeyManager == "CUSTOMER") {
                    await context.sdkcall("KMS", "getKeyPolicy", {
                        KeyId: key.KeyId,
                        PolicyName: "default"
                    }, true).then(async (data) => {
                        keydata.KeyMetadata['Policy'] = data.Policy;

                        await context.sdkcall("KMS", "getKeyRotationStatus", {
                            KeyId: key.KeyId
                        }, false).then((data) => {
                            keydata['KeyRotationEnabled'] = data.KeyRotationEnabled;
                        }).catch(() => { });

                        keydata.KeyMetadata['Tags'] = await context.getResourceTags(keydata.KeyMetadata.Arn);
    ...
    await context.sdkcall("KMS", "listAliases", {
        // no params
    }, true).then((data) => {
