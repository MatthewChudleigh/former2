...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("Macie2", "getMacieSession", {
        // no params
    ...
    await context.sdkcall("Macie2", "listCustomDataIdentifiers", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.items.map(async (customDataIdentifer) => {
            await context.sdkcall("Macie2", "getCustomDataIdentifier", {
    ...
    await context.sdkcall("Macie2", "listFindingsFilters", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.findingsFilterListItems.map(async (findingsFilter) => {
            await context.sdkcall("Macie2", "getFindingsFilter", {
    ...
    await context.sdkcall("Macie", "listMemberAccounts", {
        // no params
    }, false).then(async (data) => {
    ...
    await context.sdkcall("Macie", "listS3Resources", {
        // no params
    }, false).then(async (data) => {
