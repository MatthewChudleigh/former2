...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("AppRunner", "listServices", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.ServiceSummaryList.map(async (service) => {
            return context.sdkcall("AppRunner", "describeService", {
    ...
    await context.sdkcall("AppRunner", "listObservabilityConfigurations", {
        LatestOnly: true
    }, true).then(async (data) => {
        await Promise.all(data.ObservabilityConfigurationSummaryList.map(async (observabilityconfiguration) => {
            return context.sdkcall("AppRunner", "describeObservabilityConfiguration", {
