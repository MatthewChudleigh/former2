...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("Inspector2", "listFilters", {
        // no params
    ...
    await context.sdkcall("Inspector", "listAssessmentTargets", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.assessmentTargetArns.map(assessmentTargetArn => {
            return Promise.all([
                context.sdkcall("Inspector", "describeAssessmentTargets", {
                    ...
                    if (data.assessmentTargets[0].resourceGroupArn) {
                        await context.sdkcall("Inspector", "describeResourceGroups", {
                ...
                context.sdkcall("Inspector", "listAssessmentTemplates", {
                    assessmentTargetArns: [assessmentTargetArn]
                }, true).then(async (data) => {
                    await Promise.all(data.assessmentTemplateArns.map(assessmentTemplateArn => {
                        return context.sdkcall("Inspector", "describeAssessmentTemplates", {
