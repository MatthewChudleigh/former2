...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("GuardDuty", "listDetectors", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.DetectorIds.map(detectorId => {
            return Promise.all([
                context.sdkcall("GuardDuty", "listMembers", {
                ...
                context.sdkcall("GuardDuty", "listFilters", {
                    DetectorId: detectorId
                }, true).then(async (data) => {
                    await Promise.all(data.FilterNames.map(filterName => {
                        return context.sdkcall("GuardDuty", "getFilter", {
                            DetectorId: detectorId,
                ...
                context.sdkcall("GuardDuty", "listIPSets", {
                    DetectorId: detectorId
                }, true).then(async (data) => {
                    await Promise.all(data.IpSetIds.map(ipSetId => {
                        return context.sdkcall("GuardDuty", "getIPSet", {
                            DetectorId: detectorId,
                            IpSetId: ipSetId
                        }, true).then((data) => {
                            data['DetectorId'] = detectorId;
                ...
                context.sdkcall("GuardDuty", "listThreatIntelSets", {
                    DetectorId: detectorId
                }, true).then(async (data) => {
                    await Promise.all(data.ThreatIntelSetIds.map(threatIntelSetId => {
                        return context.sdkcall("GuardDuty", "getThreatIntelSet", {
                            DetectorId: detectorId,
                ...
                context.sdkcall("GuardDuty", "getMasterAccount", {
                    DetectorId: detectorId
                }, true).then((data) => {
                    if (data.Master) {
                ...
                context.sdkcall("GuardDuty", "getDetector", {
                    DetectorId: detectorId
                }, true).then((data) => {
                    resources.push({
                        f2id: detectorId,
