...
async function updateDatatable(context) {
    ...
    if (["us-east-1", "us-west-2", "eu-west-1"].includes(context.region)) { // has to be whitelisted otherwise it hangs on the API call
        ...
        await context.sdkcall("SES", "listReceiptRuleSets", {
            // no params
            ...
            await Promise.all(data.RuleSets.map(ruleSet => {
                ...
                return context.sdkcall("SES", "describeReceiptRuleSet", {
        ...
        await context.sdkcall("SESV2", "listDedicatedIpPools", {
            // no params
        }, true).then((data) => {
        ...
        await context.sdkcall("SESV2", "listEmailIdentities", {
            // no params
        }, true).then(async (data) => {
            await Promise.all(data.EmailIdentities.map(emailidentity => {
                return context.sdkcall("SESV2", "getEmailIdentity", {
    ...
    if ([
        ...
        await context.sdkcall("SES", "listConfigurationSets", {
            ...
            await Promise.all(data.ConfigurationSets.map(configurationSet => {
                return context.sdkcall("SES", "describeConfigurationSet", {
        ...
        await context.sdkcall("SES", "listTemplates", {
            // no params
        }, true).then(async (data) => {
            await Promise.all(data.TemplatesMetadata.map(template => {
                return context.sdkcall("SES", "getTemplate", {
        ...
        await context.sdkcall("SESV2", "listContactLists", {
            // no params
        }, true).then(async (data) => {
            await Promise.all(data.ContactLists.map(contactlist => {
                return context.sdkcall("SESV2", "getContactList", {
