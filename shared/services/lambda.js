...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("Lambda", "listFunctions", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.Functions.map(async (lambdaFunction) => {
            return Promise.all([
                context.sdkcall("Lambda", "getFunction", {
                ...
                context.sdkcall("Lambda", "getPolicy", {
                    FunctionName: lambdaFunction.FunctionArn
                }, false).then((data) => {
                ...
                context.sdkcall("Lambda", "listAliases", {
                    FunctionName: lambdaFunction.FunctionArn
                }, true).then(async (data) => {
                    await Promise.all(data.Aliases.map(async (alias) => {
                        await context.sdkcall("Lambda", "getProvisionedConcurrencyConfig", {
                            FunctionName: lambdaFunction.FunctionArn,
                ...
                context.sdkcall("Lambda", "listVersionsByFunction", {
                    FunctionName: lambdaFunction.FunctionArn
                }, true).then(async (data) => {
                ...
                context.sdkcall("Lambda", "listFunctionEventInvokeConfigs", {
                    FunctionName: lambdaFunction.FunctionArn
                ...
                context.sdkcall("Lambda", "listFunctionUrlConfigs", {
                    FunctionName: lambdaFunction.FunctionArn
                }, false).then(async (data) => {
                    var urlIterator = 1;
    ...
    await context.sdkcall("Lambda", "listLayers", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.Layers.map(layer => {
            return context.sdkcall("Lambda", "listLayerVersions", {
                LayerName: layer.LayerName
            }, true).then(async (data) => {
                await Promise.all(data.LayerVersions.map(layerVersion => {
                    return Promise.all([
                        context.sdkcall("Lambda", "getLayerVersion", {
                            LayerName: layer.LayerName,
                        ...
                        context.sdkcall("Lambda", "getLayerVersionPolicy", {
                            LayerName: layer.LayerName,
                            VersionNumber: layerVersion.Version
                        }, false).then((data) => {
                            var policy = JSON.parse(data.Policy);
    ...
    await context.sdkcall("Lambda", "listEventSourceMappings", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.EventSourceMappings.map(eventSourceMapping => {
            return context.sdkcall("Lambda", "getEventSourceMapping", {
    ...
    await context.sdkcall("Lambda", "listCodeSigningConfigs", {
        // no params
    }, false).then(async (data) => {
        await Promise.all(data.CodeSigningConfigs.map(codeSigningConfig => {
            return context.sdkcall("Lambda", "getCodeSigningConfig", {
