...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("EFS", "describeFileSystems", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.FileSystems.map(async (fileSystem) => {
            await context.sdkcall("EFS", "describeFileSystemPolicy", {
                FileSystemId: fileSystem.FileSystemId
            }, false).then(async (policydata) => {
                fileSystem['BackupPolicy'] = policydata.BackupPolicy;
            }).catch(err => { });

            await context.sdkcall("EFS", "describeBackupPolicy", {
                FileSystemId: fileSystem.FileSystemId
            }, false).then(async (policydata) => {
            ...
            return Promise.all([
                context.sdkcall("EFS", "describeMountTargets", {
                    FileSystemId: fileSystem.FileSystemId
                }, true).then(async (data) => {
                    await Promise.all(data.MountTargets.map(mountTarget => {
                        return context.sdkcall("EC2", "describeNetworkInterfaces", {
                ...
                context.sdkcall("EFS", "describeAccessPoints", {
                    FileSystemId: fileSystem.FileSystemId
                }, false).then((apdata) => {
