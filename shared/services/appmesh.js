...
async function updateDatatable(context) {
    const resources = [];
    await context.sdkcall("AppMesh", "listMeshes", {
        // no params
    }, true).then(async (data) => {
        await Promise.all(data.meshes.map(mesh => {
            return Promise.all([
                context.sdkcall("AppMesh", "describeMesh", {
                ...
                context.sdkcall("AppMesh", "listVirtualRouters", {
                    meshName: mesh.meshName
                }, true).then(async (data) => {
                    await Promise.all(data.virtualRouters.map(virtualrouter => {
                        return Promise.all([
                            context.sdkcall("AppMesh", "describeVirtualRouter", {
                                meshName: mesh.meshName,
                            ...
                            context.sdkcall("AppMesh", "listRoutes", {
                                meshName: mesh.meshName,
                                virtualRouterName: virtualrouter.virtualRouterName
                            }, true).then(async (data) => {
                                await Promise.all(data.routes.map(route => {
                                    return context.sdkcall("AppMesh", "describeRoute", {
                                        meshName: mesh.meshName,
                                        virtualRouterName: virtualrouter.virtualRouterName,
                ...
                context.sdkcall("AppMesh", "listVirtualServices", {
                    meshName: mesh.meshName
                }, true).then(async (data) => {
                    await Promise.all(data.virtualServices.map(virtualservice => {
                        return context.sdkcall("AppMesh", "describeVirtualService", {
                            meshName: mesh.meshName,
                ...
                context.sdkcall("AppMesh", "listVirtualNodes", {
                    meshName: mesh.meshName
                }, true).then(async (data) => {
                    await Promise.all(data.virtualNodes.map(virtualnode => {
                        return context.sdkcall("AppMesh", "describeVirtualNode", {
                            meshName: mesh.meshName,
                ...
                context.sdkcall("AppMesh", "listVirtualGateways", {
                    meshName: mesh.meshName
                }, true).then(async (data) => {
                    await Promise.all(data.virtualGateways.map(async (virtualgateway) => {
                        await context.sdkcall("AppMesh", "describeVirtualService", {
                            meshName: mesh.meshName,
                        ...
                        return context.sdkcall("AppMesh", "listGatewayRoutes", {
                            meshName: mesh.meshName,
                            virtualGatewayName: virtualgateway.virtualGatewayName
                        }, true).then(async (data) => {
                            await Promise.all(data.gatewayRoutes.map(async (gatewayroute) => {
                                await context.sdkcall("AppMesh", "describeGatewayRoute", {
                                    meshName: mesh.meshName,
                                    virtualGatewayName: virtualgateway.virtualGatewayName,
